var auditOrderVM = new Vue({
	...
	methods : {
		...
		confirmToPaid : function(orderId) {//确认到账
			var that = this;
			layer.confirm('确认收到款了吗?', {
				title : '确认收到款了吗?',
				content:  '<div class="form-group">'+
						'<label class=" control-label">备注</label>'+
						'<textarea id="confirmText" class="form-control" rows="3"></textarea>'+
						'</div>',
				btn : [ '确认', '返回' ]
			}, function(index) {
				let textValuedesk= $('#confirmText').val();
				//alert("弹出提示数据:="+textValuedesk);
				//textValuedesk : text;
				if (textValuedesk == null || textValuedesk == '') {
					layer.alert('请输入备注描述信息', {
						title : '提示',
						icon : 7,
						time : 3000
					});
				...
				layer.close(index);
				that.confirmToPaidInner(orderId,textValuedesk);//调用接口
			}, function(index) {
				layer.close(index);
			});
		...
		confirmToPaidInner : function(orderId,textValuedesk) {
			var that = this;
			that.$http.get('/merchantOrder/userConfirmToPaid', {
				params : {
					orderId : orderId,
					textValuedesk : textValuedesk
				}
			}).then(function(res) {
				layer.alert('操作成功', {
					icon : 1,
