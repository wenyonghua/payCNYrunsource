var merchantOrderVM = new Vue({
	...
	methods : {
			...
			this.currentTable = $('.platform-order-table').bootstrapTable({
				...
				columns : [ {
					...
					},
					{
						title : '收款金额',
						formatter : function(value, row, index, field) {
							//var text = row.gatheringAmount + that.global.systemSetting.currencyUnit;
							var text = row.gatheringAmountView +" "+that.global.systemSetting.currencyUnit;
							return text;
						}
					},
					{
		...
		resendNotice : function(id) {
			...
			that.$http.get('/merchantOrder/resendMerchantNotice', {
				params : {
					id : id
				}
			}).then(function(res) {
		...
		orderloadDown : function() {//下载数据
			...
			//window.open('/merchantOrder/merchantOrderExport?orderNo='+that.orderNo+'&merchantOrderNo='+that.merchantOrderNo+'&orderState='+that.orderState+'&receiverUserName='+that.receiverUserName+"&submitStartTime="+that.submitStartTime+"&submitEndTime="+that.submitEndTime+"&pageNum="+1+"&pageSize="+99999999);
			//使用新方法
			window.open('/merchantOrder/merchantOrdernewExport?orderNo='+that.orderNo+'&merchantOrderNo='+that.merchantOrderNo+'&orderState='+that.orderState+'&receiverUserName='+that.receiverUserName+"&submitStartTime="+that.submitStartTime+"&submitEndTime="+that.submitEndTime+"&pageNum="+1+"&pageSize="+99999999);
		},
