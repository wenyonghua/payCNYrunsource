var accountManageVM = new Vue({
	...
	data : {
		...
		ipWhitelist :'',
		systemSource : '',
		receiveOrderDictItems :[],
		receiveOrderState :'',
		payerMark :'',//付款人员标记
		payerMarkName :'',//付款人员标记名称
		paymentRange : '',//付款范围
	...
	mounted : function() {
		...
		this.loadReceiveOrderStateTypeItem();//接单状态
		this.loadAccountReceiveOrderChannelStateItem();
		this.loadGatheringChannelDictItem();
		this.initTable();
		this.payerMarkList();//付款标记
	},
	methods : {

		loadAccountTypeItem : function() {//查询账号类型
			var that = this;
		...
		loadReceiveOrderStateTypeItem : function() {//查询接单状态
			var that = this;
			that.$http.get('/dictconfig/findDictItemInCache', {
				params : {
					dictTypeCode : 'receiveOrderState'
				}
			}).then(function(res) {
				this.receiveOrderDictItems = res.body.data;
			});
		},

		payerMarkList : function() {//付款标记状态
			var that = this;
			that.$http.get('/dictconfig/findDictItemInCache', {
				params : {
					dictTypeCode : 'PayerMark'
				}
			}).then(function(res) {
				this.payerMarkListDictItems = res.body.data;
			});
		},
			...
			$('.account-manage-table').bootstrapTable({
				...
				queryParams : function(params) {
					var condParam = {
						...
						receiveOrderState: that.receiveOrderState,//接单状态
						payerMark : that.payerMark
					};
					console.log(params);
					return condParam;
				},
