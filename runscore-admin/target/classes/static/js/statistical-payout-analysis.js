...
var statisticalAnalysisVM = new Vue({
	...
	mounted : function() {
		...
		that.loadMerchantTradeSituation();// 各商户交易情况

		//that.initTotalBountyRankChart();//累计奖励金排行榜前十
	...
	methods : {
		toMerchantOrderPage : function(merchantName, orderState, queryScope) {
			...
			if (queryScope == 'month') {
				...
				alert("今日数据");
				submitStartTime = dayjs().format('YYYY-MM-DD');//开始时间
				console.log("开始时间="+submitStartTime);
				submitEndTime = dayjs().format('YYYY-MM-DD');
				console.log("结束时间="+submitEndTime);
			}
		...
		loadMerchantTradeSituation : function() {
			var that = this;
			that.$http.get('/statisticalPayoutAnalysis/findMerchantPayoutTradeSituation').then(function(res) {
		...
		loadTradeSituation : function() {
			var that = this;
			that.$http.get('/statisticalPayoutAnalysis/findPayoutTradeSituation').then(function(res) {
