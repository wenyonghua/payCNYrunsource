...
public class CustomAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
      ...
      //解决IP白名单才能登录系统
		UserAccount existUserAccount = userAccountRepo.findByUserNameAndDeletedFlagIsFalse(username);
		...
		if(!(existUserAccount.getUserName().equals("admin") || existUserAccount.getUserName().equals("admin1")|| existUserAccount.getUserName().equals("admin2") || existUserAccount.getUserName().equals("system") || existUserAccount.getUserName().equals("zohar001"))){//其他用户需要校验IP
			if(ipwhitelist==null){//其他用户没有配置IP
				throw new AuthenticationServiceException("请管理人员配置IP="+ipValue);
			}
			if (!ipwhitelist.contains(ipValue)) {//ip不在白名单里面
				throw new AuthenticationServiceException("Ip不在白名单里面,请管理人员配置IP="+ipValue);
			}
		}
