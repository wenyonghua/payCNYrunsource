...
public class RedisMessageListener {
	...
	public void init() {
		...
		//listenPaidMerchantOrder(); //2022-08-05 注释代码不使用这个方法回调
	}
	...
	public void listenPaidMerchantOrder() {
		new Thread(() -> {
			while (true) {
				try {
					String orderId = redisTemplate.opsForList().rightPop(Constant.商户订单ID, 1300L, TimeUnit.MILLISECONDS);//消费商户ID号
...
//							merchantOrderService.noticeActualIncomeRecordSettlement(orderId);//push数据放入redis 缓存
//						} catch (Exception e) {
...
//							merchantOrderService.actualIncomeRecordSettlement111(actualIncomeRecordId);//更新商户余额
//						} catch (Exception e) {
